...
def run_harvest():
    ...
    if new_results:
        ...
        print("\n[!] Harvest completed but no new data was collected today.")

    # ==========================================
    # ALPHA ENGINE SCORING (Runs Every Time)
    # ==========================================
    if os.path.exists(FILE_NAME):
        ...
        def score_options(group):
            ...
            std_10d = group['M2_DeltaNetOI'].rolling(window=10, min_periods=1).std().replace(0, 1) 
            ...
            for z, is_sat in zip(group['Z_Score'], is_saturday):
                if is_sat:
                    delta_scores.append(0) # Exclude Saturday expiries
                elif z > 2:
                    delta_scores.append(1) # +2 SD Spike
                elif z < -2:
                    delta_scores.append(-1) # -2 SD Crash
                else:
                    delta_scores.append(0) # Normal daily flow
                    
            group['Opt_DeltaOI_Score'] = delta_scores
            return group

        # 3. Apply the scoring math
        df_scored = df_history.groupby('Ticker', group_keys=False).apply(score_options)
        
        # 4. Isolate the most recent day's scores
        ...
        # 5. Save the lightweight parquet
