    ...
    with c_bot_right:
        ...
        if 'df_losers' in locals() and not df_losers.empty and active_etfs:
            ...
            if not df_fsli.empty:
                # Ultra-defensive merge: Extract ONLY the Ticker and _Score columns
                # This guarantees we never get a KeyError: 'Index'
                score_cols = ['Ticker'] + [c for c in df_fsli.columns if c.endswith('_Score')]
                valid_cols = [c for c in score_cols if c in df_fsli.columns]
                df_fsli_scores = df_fsli[valid_cols]
                alpha_df = alpha_df.merge(df_fsli_scores, on='Ticker', how='left')

            # --- 2. MERGE THE OPTIONS SCORES ---
            try:
                # Read the lightweight options score file generated by your harvester
                df_opt_scores = pd.read_parquet('options_scores_latest.parquet')
                if not df_opt_scores.empty:
                    alpha_df = alpha_df.merge(df_opt_scores, on='Ticker', how='left')
            except Exception:
                pass
            ...
            for col_tuple in display_df.columns:
                ...
                # Let the concatenated columns scale naturally
                elif col_tuple[0] in ["VALUE", "PROFIT", "OPT"]:
                    strict_col_config[col_key] = st.column_config.TextColumn(width="content")
                # Crush the single emoji columns to absolute minimum width (1 pixel hack)
                else:
        ...
        with t_news:
            ...
            if available_news_tickers:
                ...
                if not df_news.empty:
                    ...
                    fig_news.update_layout(margin=dict(l=0,r=10,t=5,b=0), height=TAB_HEIGHT - 50 if 'TAB_HEIGHT' in locals() else 590)
