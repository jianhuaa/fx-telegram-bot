...
# ============================================================
# DATA FUNCTIONS (all cached, all remote)
# ============================================================
GITHUB_BASE = "https://raw.githubusercontent.com/jianhuaa/fx-telegram-bot/main/"
...
def get_historical_options_data(ticker):
    try:
        ...
        df = pd.read_parquet(GITHUB_BASE + "col4_options_history.parquet")
...
def get_sector_data(tf):
    try:
        tf_period_map = {"1D": "1d", "1W": "5d", "1M": "1mo", "3M": "3mo", "1Y": "1y"}
        tf_int_map = {"1D": "1m", "1W": "5m", "1M": "15m", "3M": "1d", "1Y": "1d"}
        sector_map = {'XLK': 'XLK', 'XLF': 'XLF', 'XLV': 'XLV', 'XLY': 'XLY', 'XLP': 'XLP',
                      'XLE': 'XLE', 'XLI': 'XLI', 'XLU': 'XLU', 'XLB': 'XLB', 'XLRE': 'XLRE', 'XLC': 'XLC'}
...
def get_cme_historical_data():
    try:
        df = pd.read_csv(GITHUB_BASE + "spdr_sectors_history.csv")
        df['Date'] = pd.to_datetime(df['Date'])
        recent_dates = sorted(df['Date'].unique(), reverse=True)[:5]
        df_recent = df[df['Date'].isin(recent_dates)].sort_values('Date')
        cme_xl_map = {'COMM': 'XLC', 'DISC': 'XLY', 'ENER': 'XLE', 'FINA': 'XLF', 'HLTH': 'XLV',
                      'INDU': 'XLI', 'MATL': 'XLB', 'REIT': 'XLRE', 'STAP': 'XLP', 'TECH': 'XLK', 'UTIL': 'XLU'}
...
def get_sp500_master_data():
    try:
        df = pd.read_csv(GITHUB_BASE + "sp500_history.csv")
        df['Date'] = pd.to_datetime(df['Date'])
        recent_dates = sorted(df['Date'].unique(), reverse=True)[:5]
        df_recent = df[df['Date'].isin(recent_dates)].sort_values('Date')
        latest_date = df_recent['Date'].max()
        df_fut = df_recent[df_recent['Type'] == 'FUT'].copy()
        front_2_fut = sorted(df_fut[df_fut['Date'] == latest_date]['Month'].unique(), key=get_month_score)[:2]
        df_fut_front = df_fut[df_fut['Month'].isin(front_2_fut)].copy()
        df_opt = df_recent[df_recent['Type'] == 'OPT'].copy()
        front_2_opt = sorted(df_opt[df_opt['Date'] == latest_date]['Month'].unique(), key=get_month_score)[:2]
...
def get_rut_master_data():
    try:
        df = pd.read_csv(GITHUB_BASE + "russell_history.csv")
        df['Date'] = pd.to_datetime(df['Date'])
        recent_dates = sorted(df['Date'].unique(), reverse=True)[:5]
        df_recent = df[df['Date'].isin(recent_dates)].sort_values('Date')
        latest_date = df_recent['Date'].max()
    ...
    try:
        def process_sheet(url, cap_label, start_row):
            ...
            def clean_col(col_name):
                ...
                s = df[col_name].astype(str).str.replace(r'[^0-9\.\-]', '', regex=True).replace('', float('nan'))
            ...
            c_1d = get_closest(1); c_1w = get_closest(7); c_1m = get_closest(30)
            c_3m = get_closest(91); c_1y = get_closest(365)
            ...
            def map_sec(x):
                ...
                sec_map_inv = {"COMMUNICATION": "XLC", "DISCRETIONARY": "XLY", "STAPLES": "XLP",
        ...
        res = df_all.groupby(['ETF', 'IND_A', 'Cap']).agg(
            {'1D_raw': 'mean', '1W_raw': 'mean', '1M_raw': 'mean', '3M_raw': 'mean', '1Y_raw': 'mean', 'Symbol': 'count'}
        ).reset_index()
...
def get_all_insider_trades():
    try:
        return pd.read_parquet(GITHUB_BASE + "col4_insider_trades.parquet")
    except Exception as e:
    ...
    try:
        ...
        if num_rows > 0:
            ...
            for display_name, raw_key in field_mapping.items():
                val = one_y_val if display_name == '1Y%' else (
                    short_interest_val if display_name == 'Short Interest %' else (
                        last_price if display_name == 'Last Price' else df[raw_key].values[0]))
...
def get_dynamic_options_data(ticker):
    ...
    try:
        ...
        for date in expirations:
            target = "M1" if date.startswith(m1_str) else ("M2" if date.startswith(m2_str) else None)
            if target:
        ...
        return {
            "m1_name": m1_name, "m1_sigma": stats["M1_Call_Vol"] - stats["M1_Put_Vol"],
            "m1_delta": 0, "m1_pc": stats["M1_Put_Vol"] / stats["M1_Call_Vol"] if stats["M1_Call_Vol"] > 0 else 0,
            "m2_name": m2_name, "m2_sigma": stats["M2_Call_Vol"] - stats["M2_Put_Vol"],
            "m2_delta": 0, "m2_pc": stats["M2_Put_Vol"] / stats["M2_Call_Vol"] if stats["M2_Call_Vol"] > 0 else 0
        }
    except: return None
...
def get_live_col4_data():
    try:
        def safe_read_remote(fname):
            try:
                res = requests.get(GITHUB_BASE + fname)
        ...
        df_all_ret     = safe_read_remote('col4_all_returns.parquet')  # Pushed externally
...
def show_summary_overlay(default_tf, default_sec, df_all_ret):
    ...
    with st.spinner("Loading timeframe data..."):
        ...
        if not df_history.empty:
            ...
            if t_vals_d[-1] != last_date_d: t_vals_d[-1] = last_date_d
    ...
    with col_left:
        ...
        with c1:
            ...
            if not df_pct.empty and 'SPX' in df_pct and 'RUT' in df_pct:
                ...
                f1.update_layout(title=dict(text=f"SPX vs RTY ({local_tf})", x=0.0, y=0.99, yref="container", xanchor="left", yanchor="top", font=dict(size=14, color="white")), xaxis=dialog_x_axis, yaxis=dict(showgrid=False, ticksuffix="%"), legend=dict(orientation="h", yanchor="bottom", y=1.06, xanchor="left", x=0.0, bgcolor="rgba(0,0,0,0)", font=dict(size=12)), margin=dict(l=0, r=0, t=70, b=0), height=300)
        ...
        with c2:
            ...
            if not df_sectors_pct.empty and not df_pct.empty:
                ...
                for sec_col in df_sectors_pct.columns:
                    is_visible = True if sec_col == local_sec else 'legendonly'
                    f2.add_trace(go.Scatter(x=df_sectors_pct.index, y=df_sectors_pct[sec_col], name=sec_col, visible=is_visible))
                ...
                f2.update_layout(title=dict(text="SPX vs Sectors", x=0.0, y=0.99, yref="container", xanchor="left", yanchor="top", font=dict(size=14, color="white")), xaxis=dialog_x_axis, yaxis=dict(showgrid=False, ticksuffix="%"), legend=dict(orientation="h", yanchor="bottom", y=0.95, xanchor="left", x=0.0, bgcolor="rgba(0,0,0,0)", font=dict(size=10), itemwidth=30), margin=dict(l=0, r=0, t=70, b=0), height=300)
        ...
        if not df_inds.empty:
            ...
            idx_map_cap = {'L': 'SPX', 'M': 'RMC', 'S': 'RTY'}
            for _, row_ind in df_inds_filtered.iterrows():
                mixed_data.append({
                    'Type': 'Industry', 'Index': idx_map_cap.get(row_ind['Cap'], row_ind['Cap']),
                    'Sector': row_ind['ETF'], 'Industry': str(row_ind['IND_A']).replace('<br>', ' '),
                    'Count': int(row_ind['N']),
            ...
            if not df_mixed.empty and sort_col_mixed in df_mixed.columns:
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "Count": st.column_config.NumberColumn("Count", width=35, format="%d"),
                        "1D%": st.column_config.TextColumn(width=55), "1W%": st.column_config.TextColumn(width=55),
                        "1M%": st.column_config.TextColumn(width=60), "3M%": st.column_config.TextColumn(width=60),
        ...
        with t2:
            if not df_all_ret.empty:
                ...
                if local_ind != "Unknown": df_t = df_t[df_t['Industry'] == local_ind]
                if sort_col_mixed in df_t.columns and not df_t.empty:
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            "Ticker": st.column_config.TextColumn(width=40), "Index": st.column_config.TextColumn(width=40),
                            "Sector": st.column_config.TextColumn(width=40), "Industry": st.column_config.TextColumn(width=290),
                            "1D%": st.column_config.TextColumn(width=55), "1W%": st.column_config.TextColumn(width=55),
                            "1M%": st.column_config.TextColumn(width=55), "3M%": st.column_config.TextColumn(width=60),
                            "1Y%": st.column_config.TextColumn(width=60),
                        }
                    )
                else:
    ...
    with col_right:
        f3 = go.Figure()
        sort_col_chart = f"{local_tf}_raw"
        has_data = False
        ...
        x_vals, y_labels, all_vals = [], [], [0]

        if not df_inds.empty and sort_col_chart in df_inds.columns:
            ind_sub_raw = df_inds[df_inds['ETF'] == local_sec].copy()
            if not ind_sub_raw.empty:
                ind_sub = ind_sub_raw.groupby('IND_A')[sort_col_chart].mean().reset_index().sort_values(sort_col_chart)
                if not ind_sub.empty:
                    has_data = True
                    y_labels_raw = ind_sub['IND_A'].str.replace('<br>', ' ').tolist()
                    y_labels = ["<br>".join(textwrap.wrap(label, width=20)) if label.count(' ') >= 2 else label for label in y_labels_raw]
                    x_vals = ind_sub[sort_col_chart].tolist()
                    all_vals = x_vals + [spx_ret, sec_ret, 0]
                    v_min, v_max = min(all_vals), max(all_vals)
                    v_range = v_max - v_min
                    padding = v_range * 0.30 if v_range > 0 else 2.0

                    f3.add_trace(go.Bar(x=x_vals, y=y_labels, orientation='h', marker_color='#555555',
                                        text=[f"{v:+.1f}%" for v in x_vals], hoverinfo='x+y', textposition='none'))
                    for y, x in zip(y_labels, x_vals):
                        f3.add_annotation(x=max(0, x) + (max(x_vals) * 0.05), y=y, text=f"{x:+.1f}%",
                                          showarrow=False, xanchor='left' if x >= 0 else 'right',
                                          xshift=8 if x >= 0 else -8, font=dict(size=12, color="white"),
                                          bgcolor="rgba(20, 20, 20, 0.85)", bordercolor="#555", borderwidth=1, borderpad=3)
                    f3.add_vline(x=spx_ret, line_dash="dot", line_color="#00aaff",
                                 annotation_text=f"SPX ({spx_ret:+.1f}%)", annotation_position="top right",
                                 annotation_font=dict(color="white", size=13),
                                 annotation_bgcolor="rgba(0, 100, 200, 0.9)",
                                 annotation_bordercolor="#00aaff", annotation_borderwidth=2, annotation_borderpad=6, annotation_y=1.08)
                    f3.add_vline(x=sec_ret, line_dash="dash", line_color="#f4ca16",
                                 annotation_text=f"{local_sec} ({sec_ret:+.1f}%)", annotation_position="bottom right",
                                 annotation_font=dict(color="white", size=13),
                                 annotation_bgcolor="rgba(180, 140, 0, 0.9)",
                                 annotation_bordercolor="#f4ca16", annotation_borderwidth=2, annotation_borderpad=6, annotation_y=-0.08)
        ...
        if not has_data:
            ...
            f3.update_layout(
                ...
                xaxis=dict(range=[v_min - padding, v_max + padding], showgrid=False, ticksuffix="%", zeroline=True, zerolinecolor='white', ticklabelstandoff=40, side="bottom"),
                yaxis=dict(showgrid=False, tickfont=dict(size=11), domain=[0.15, 1.0]),
                margin=dict(l=0, r=10, t=85, b=20), height=640, showlegend=False,
...
def show_industry_overview_overlay(df_all_returns, df_industries, selected_sector_default, df_sec_live, df_transcripts_live, df_history, df_sectors_price, passed_industry_default=None):
    ...
    with c2:
        sel_ind = st.selectbox("Industry", inds if inds else ["Unknown"], index=idx_ind, label_visibility="collapsed", key=dyn_ind_key)
    ...
    with c3:
        selected_labels = st.multiselect("Tickers", available_tickers, default=available_tickers[:3] if len(available_tickers) >= 3 else available_tickers, label_visibility="collapsed", placeholder="Select tickers for FSLI/Options...")
    st.markdown("<hr style='margin:10px 0; border-color:#333;'>", unsafe_allow_html=True)
    FSLI_H = 680; MACRO_H = 215; INS_H = 250
    ...
    with right_col:
        def get_pct_returns(df, c):
            if not df.empty and c in df.columns:
                try:
                    return [
                        round((df[c].iloc[-1] / df[c].iloc[-2] - 1) * 100, 2) if len(df) > 1 else 0.0,
                        round((df[c].iloc[-1] / df[c].iloc[-6] - 1) * 100, 2) if len(df) > 5 else 0.0,
                        round((df[c].iloc[-1] / df[c].iloc[-22] - 1) * 100, 2) if len(df) > 21 else 0.0,
                        round((df[c].iloc[-1] / df[c].iloc[-64] - 1) * 100, 2) if len(df) > 63 else 0.0,
                    ]
                except: pass
            return [0.0, 0.0, 0.0, 0.0]

        # Use 1Y data for macro/micro pct returns
        df_history_1y, _ = get_historical_charts_data("1Y")
        df_sectors_1y, _ = get_sector_data("1Y")
        spx_vals = get_pct_returns(df_history_1y, 'SPX')
        rut_vals = get_pct_returns(df_history_1y, 'RUT')
        sec_vals = get_pct_returns(df_sectors_1y, sel_sec)
        ...
        with t_mac:
            ...
            if not match_df.empty:
                ...
                for idx_r, row in match_df.sort_values('Type_Cat').iterrows():
                    n_count = int(row['N']); clean_name = str(row['IND_A']).replace('<br>', ' ')
            ...
            fig_mac_tbl = go.Figure(data=[go.Table(columnwidth=[160, 40, 40, 40, 40], header=dict(values=['<b>INDEX/ETF</b>', '<b>1D</b>', '<b>1W</b>', '<b>1M</b>', '<b>3M</b>'], fill_color='#161616', font=dict(color='#f4ca16', size=11), align=['left', 'right', 'right', 'right', 'right'], height=24), cells=dict(values=[macro_names, macro_1d, macro_1w, macro_1m, macro_3m], fill_color='#0d0d0d', font=dict(color='white', size=11), align=['left', 'right', 'right', 'right', 'right'], height=26))])
        ...
        with t_mic:
            ...
            if not ind_df.empty:
                ...
                fig_scat = go.Figure()
                if spx_vals[2] or spx_vals[1]: fig_scat.add_trace(go.Scatter(x=[round(spx_vals[2], 2)], y=[round(spx_vals[1], 2)], mode='markers', marker=dict(symbol='star', size=12, color='#ffffff', line=dict(width=1, color='black')), name='SPX'))
                if rut_vals[2] or rut_vals[1]: fig_scat.add_trace(go.Scatter(x=[round(rut_vals[2], 2)], y=[round(rut_vals[1], 2)], mode='markers', marker=dict(symbol='star', size=12, color='#888888', line=dict(width=1, color='black')), name='RTY'))
                if sec_vals[2] or sec_vals[1]: fig_scat.add_trace(go.Scatter(x=[round(sec_vals[2], 2)], y=[round(sec_vals[1], 2)], mode='markers', marker=dict(symbol='star', size=12, color='#f4ca16', line=dict(width=1, color='black')), name=sel_sec))
                if not match_df.empty:
                    for idx_r, row in match_df.sort_values('Type_Cat').iterrows():
                        name_str = f"Avg ({row['Type']})"; color = '#00aaff' if row['Type'] == 'SPX' else ('#ff5252' if row['Type'] == 'RMC' else '#ab63fa')
                        fig_scat.add_trace(go.Scatter(x=[round(row['1M_raw'], 2)], y=[round(row['1W_raw'], 2)], mode='markers', marker=dict(symbol='diamond', size=10, color=color, line=dict(width=1, color='white')), name=name_str))
                for idx_name, c_hex in [('SPX', '#00aaff'), ('RMC', '#ff5252'), ('RTY', '#ab63fa')]:
                    s_df = plot_df[plot_df['Index'] == idx_name]
                    if not s_df.empty: fig_scat.add_trace(go.Scatter(x=s_df['1M_raw'], y=s_df['1W_raw'], mode='markers', marker=dict(color=c_hex, size=7), name=f'{idx_name} Stocks', text=s_df['Ticker']))
        ...
        t_ins, t_options, t_sec_tab, t_transcript = st.tabs(["🕵️ Insider Trades", "📉 Options", "📄 SEC Filings", "🎙️ Transcript"])
                ...
                for i, tick in enumerate(selected_tickers):
                    with ins_tabs2[i]:
                        ...
                        if not ins_df.empty:
                            ...
                            cell_vals_ins = [ins_df[c].tolist() for c in ins_df.columns]
                            col_widths_ins = [120 if c in ['Insider Name', 'Title'] else 70 for c in ins_df.columns]
                            fig_ins = go.Figure(data=[go.Table(columnwidth=col_widths_ins, header=dict(values=hdr_vals, fill_color='#161616', font=dict(color='#ff5252', size=11), align='left', height=24), cells=dict(values=cell_vals_ins, fill_color='#0d0d0d', font=dict(color='white', size=11), align='left', height=26))])
            ...
            if selected_tickers:
                ...
                for i, tick in enumerate(selected_tickers):
                    with opt_tabs[i]:
                        ...
                        if opt_data:
                            m1 = opt_data["m1_name"]; m2 = opt_data["m2_name"]; dates = opt_data["dates"]
                            y_labels_opt = [f"NetOI&nbsp;&nbsp;{m1}", f"ΔOI&nbsp;&nbsp;&nbsp;&nbsp;{m1}", f"P/C&nbsp;&nbsp;&nbsp;&nbsp;{m1}", f"NetOI&nbsp;&nbsp;{m2}", f"ΔOI&nbsp;&nbsp;&nbsp;&nbsp;{m2}", f"P/C&nbsp;&nbsp;&nbsp;&nbsp;{m2}"]
                            raw_z = [opt_data["m1_net"], opt_data["m1_delta"], opt_data["m1_pc"], opt_data["m2_net"], opt_data["m2_delta"], opt_data["m2_pc"]]
                            text_mat = [[f"{v:.2f}" if row_idx in [2, 5] else format_k(v) for v in row_vals] for row_idx, row_vals in enumerate(raw_z)]
                            fig_opt = go.Figure(data=go.Heatmap(z=[[0] * len(dates)] * 6, x=dates, y=y_labels_opt, customdata=raw_z, text=text_mat, texttemplate="%{text}", textfont=dict(size=11, color="white"), colorscale=[[0, '#161616'], [1, '#161616']], showscale=False))
                            fig_opt.add_hline(y=2.5, line_width=2, line_color="#333333")
                            fig_opt.update_layout(title=dict(text=f"{tick} Open Interest Trend", x=0.02, y=0.98, font=dict(size=12, color="white")), margin=dict(l=0, r=10, t=30, b=0), height=INS_H, yaxis=dict(autorange="reversed"))
        ...
        with t_sec_tab:
            ...
            if not sec_ind_df.empty:
                fig_sec2 = go.Figure(data=[go.Table(columnwidth=[45, 40, 35, 40, 35], header=dict(values=['<b>DATE</b>', '<b>TICK</b>', '<b>IDX</b>', '<b>TYPE</b>', '<b>LINK</b>'], fill_color='#161616', font=dict(color='#00aaff', size=11), align=['left', 'left', 'center', 'center', 'center'], height=24), cells=dict(values=[sec_ind_df['Date'], sec_ind_df['Ticker'], sec_ind_df['Index'], sec_ind_df['Type'], sec_ind_df['Link']], fill_color='#0d0d0d', font=dict(color=['white', 'white', 'white', 'white', '#00aaff'], size=11), align=['left', 'left', 'center', 'center', 'center'], height=26))])
        ...
        with t_transcript:
            ...
            if not trans_ind_df.empty:
                fig_trans2 = go.Figure(data=[go.Table(columnwidth=[45, 40, 35, 80, 40], header=dict(values=['<b>DATE</b>', '<b>TICK</b>', '<b>IDX</b>', '<b>INDUSTRY</b>', '<b>LINK</b>'], fill_color='#161616', font=dict(color='#ab63fa', size=11), align=['left', 'left', 'center', 'left', 'center'], height=24), cells=dict(values=[trans_ind_df['Date'], trans_ind_df['Ticker'], trans_ind_df['Index'], trans_ind_df['Industry'], trans_ind_df['Link']], fill_color='#0d0d0d', font=dict(color=['white', 'white', 'white', 'white', '#ab63fa'], size=11), align=['left', 'left', 'center', 'left', 'center'], height=26))])
...
if not df_history.empty:
    ...
    if t_vals[-1] != last_date: t_vals[-1] = last_date
    ...
    with c2:
        ...
        if not df_cme_recent.empty:
            df_sec_cme = df_cme_recent[df_cme_recent['XL_ID'] == selected_sector].copy()
            df_sec_cme['Date_Str'] = df_sec_cme['Date'].dt.strftime('%d %b')
            text_oi = [format_k(v) for v in df_sec_cme['OI']]
            text_delta = [format_k(v) for v in df_sec_cme['Delta']]
            fig_hm = go.Figure(data=go.Heatmap(z=[[0] * len(df_sec_cme), [0] * len(df_sec_cme)], x=df_sec_cme['Date_Str'], y=['Σ OI', 'Δ OI'], customdata=[df_sec_cme['OI'], df_sec_cme['Delta']], text=[text_oi, text_delta], texttemplate="%{text}", textfont=dict(size=13, color="white"), colorscale=[[0, '#161616'], [1, '#161616']], showscale=False))
        ...
        if len(spx_fut_months) == 2:
            ...
            df_m1 = df_spx_fut[df_spx_fut['Month'] == m1].copy()
            df_m2 = df_spx_fut[df_spx_fut['Month'] == m2].copy()
            ...
            fig_spx_f = go.Figure(data=go.Heatmap(z=[[0] * len(df_m1)] * 4, x=df_m1['Date_Str'], y=y_labels, customdata=raw_z, text=text_matrix, texttemplate="%{text}", textfont=dict(size=11, color="white"), colorscale=[[0, '#161616'], [1, '#161616']], showscale=False))
        ...
        if len(spx_opt_months) == 2:
            ...
            df_m1 = df_spx_opt[df_spx_opt['Month'] == m1].copy()
            df_m2 = df_spx_opt[df_spx_opt['Month'] == m2].copy()
            df_m1['Date_Str'] = df_m1['Date'].dt.strftime('%d %b')
            ...
            fig_spx_o = go.Figure(data=go.Heatmap(z=[[0] * len(df_m1)] * 6, x=df_m1['Date_Str'], y=y_labels, customdata=raw_z, text=text_matrix, texttemplate="%{text}", textfont=dict(size=11, color="white"), colorscale=[[0, '#161616'], [1, '#161616']], showscale=False))
    ...
    with c3:
        ...
        if not df_industries.empty:
            ...
            if not df_ind.empty:
                ...
                for col_name in ['1D', '1W', '1M', '3M']:
                    df_ind[col_name] = df_ind[f'{col_name}_raw'].apply(fmt_pct)
        ...
        if len(rut_fut_months) == 2:
            ...
            df_m1 = df_rut_fut[df_rut_fut['Month'] == m1].copy()
            df_m2 = df_rut_fut[df_rut_fut['Month'] == m2].copy()
            df_m1['Date_Str'] = df_m1['Date'].dt.strftime('%d %b')
            y_labels = [f"Σ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{m1}", f"Δ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{m1}", f"Σ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{m2}", f"Δ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{m2}"]
            raw_z = [df_m1['OI'].tolist(), df_m1['Delta'].tolist(), df_m2['OI'].tolist(), df_m2['Delta'].tolist()]
            text_matrix = [[format_k(v) for v in row] for row in raw_z]
            fig_rut_f = go.Figure(data=go.Heatmap(z=[[0] * len(df_m1)] * 4, x=df_m1['Date_Str'], y=y_labels, customdata=raw_z, text=text_matrix, texttemplate="%{text}", textfont=dict(size=11, color="white"), colorscale=[[0, '#161616'], [1, '#161616']], showscale=False))
        ...
        if len(rut_opt_months) == 2:
            ...
            df_m1 = df_rut_opt[df_rut_opt['Month'] == m1].copy()
            df_m2 = df_rut_opt[df_rut_opt['Month'] == m2].copy()
            df_m1['Date_Str'] = df_m1['Date'].dt.strftime('%d %b')
            y_labels = [f"Σ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{m1}", f"Δ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{m1}", f"P/C&nbsp;&nbsp;&nbsp;{m1}", f"Σ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{m2}", f"Δ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{m2}", f"P/C&nbsp;&nbsp;&nbsp;{m2}"]
            raw_z = [df_m1['OI'].tolist(), df_m1['Delta'].tolist(), df_m1['Sett_PC'].tolist(), df_m2['OI'].tolist(), df_m2['Delta'].tolist(), df_m2['Sett_PC'].tolist()]
            text_matrix = [[format_k(v) if i % 3 != 2 else f"{v:.2f}" for v in raw_z[i]] for i in range(6)]
            fig_rut_o = go.Figure(data=go.Heatmap(z=[[0] * len(df_m1)] * 6, x=df_m1['Date_Str'], y=y_labels, customdata=raw_z, text=text_matrix, texttemplate="%{text}", textfont=dict(size=11, color="white"), colorscale=[[0, '#161616'], [1, '#161616']], showscale=False))
    ...
    with c4:
        ...
        if not df_losers_filtered.empty:
            sort_col_losers = f"{tf_sel}_raw"
            if sort_col_losers in df_losers_filtered.columns:
                df_losers_filtered['Index_Sort'] = df_losers_filtered['Index'].map(idx_map).fillna(99)
                df_losers_filtered = df_losers_filtered.sort_values(by=[sort_col_losers, 'Industry', 'Index_Sort', 'Ticker'], ascending=[True, True, True, True]).head(50)
            for col_name, raw_col, fmt_str in [('1D', '1D_raw', '{:+.1f}%'), ('1W', '1W_raw', '{:+.0f}%'), ('1M', '1M_raw', '{:+.0f}%'), ('3M', '3M_raw', '{:+.0f}%'), ('1Y', '1Y_raw', '{:+.0f}%')]:
                df_losers_filtered[col_name] = df_losers_filtered[raw_col].apply(lambda x: fmt_str.format(x) if pd.notna(x) else "-")

        t_losers, t_sec_tab2, t_transcript2 = st.tabs(["🔴 Losers", "📄 SEC", "🎙️ Transcript"])
        TAB_HEIGHT = 640

        with t_losers:
            if not df_losers_filtered.empty:
                fig_losers = go.Figure(data=[go.Table(columnwidth=[38, 30, 32, 75, 30, 30, 30, 30, 30], header=dict(values=['<b>TICK</b>', '<b>IDX</b>', '<b>ETF</b>', '<b>INDUSTRY</b>', '<b>1D</b>', '<b>1W</b>', '<b>1M</b>', '<b>3M</b>', '<b>1Y</b>'], fill_color='#161616', font=dict(color='#ff5252', size=10), align=['left', 'center', 'center', 'left', 'center', 'center', 'center', 'center', 'center']), cells=dict(values=[df_losers_filtered['Ticker'], df_losers_filtered['Index'], df_losers_filtered['Sector'], df_losers_filtered['Industry'], df_losers_filtered['1D'], df_losers_filtered['1W'], df_losers_filtered['1M'], df_losers_filtered['3M'], df_losers_filtered['1Y']], fill_color='#0d0d0d', font=dict(color='white', size=10), align=['left', 'center', 'center', 'left', 'center', 'center', 'center', 'center', 'center'], height=28))])
        ...
        with t_transcript2:
            if not df_trans_filtered.empty:
                fig_transcripts = go.Figure(data=[go.Table(columnwidth=[45, 40, 30, 35, 80, 40], header=dict(values=['<b>DATE</b>', '<b>TICK</b>', '<b>IDX</b>', '<b>ETF</b>', '<b>INDUSTRY</b>', '<b>LINK</b>'], fill_color='#161616', font=dict(color='#ab63fa', size=11), align=['left', 'left', 'center', 'center', 'left', 'center']), cells=dict(values=[df_trans_filtered['Date'], df_trans_filtered['Ticker'], df_trans_filtered['Index'], df_trans_filtered['Sector'], df_trans_filtered['Industry'], df_trans_filtered['Link']], fill_color='#0d0d0d', font=dict(color=['white', 'white', 'white', 'white', 'white', '#ab63fa'], size=11), align=['left', 'left', 'center', 'center', 'left', 'center'], height=28))])
